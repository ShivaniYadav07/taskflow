...
export function MemberForm({ onClose }: { onClose: () => void }) {
  const { activeProject, refetchProjects } = useProjectContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await projectService.addMember(activeProject._id, email);
      toast.success('Member invited successfully!');
      await refetchProjects(); // Reload projects to get updated member list
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl animate-in zoom-in-95 fade-in">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-semibold text-slate-800">Add Member to {activeProject.name}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            />
            <p className="mt-1 text-xs text-slate-500">The user must already have an account on TaskFlow.</p>
          </div>
